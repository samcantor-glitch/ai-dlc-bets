...
4. Deploy → **New deployment** → gear icon → **Web app**.
   - Description: `AI-DLC vote intake`
   - Execute as: **Me**
...
git init -b main
git add .
git commit -m "AI-DLC vote site"
gh repo create govpilot/ai-dlc-vote --public --source=. --push
```

Then on GitHub:

- Settings → Pages → Build and deployment → Source: **Deploy from a branch** → Branch: `main` / `/ (root)` → Save.
- Wait ~1 minute. URL will be `https://govpilot.github.io/ai-dlc-vote/`.

Share that URL with engineering, PM, and QA.

## Where the results live

In the Google Sheet. One row per voter, columns:

`submitted_at | voter | role | spent | num_bets | total_units | pick_ids | pick_names | note`

Sort by `pick_ids` or pivot in another tab to tally totals by bet or by role. There is no results page on the site, votes go straight to your Sheet.

## Editing the bets

Open `index.html`, edit the `BETS` array near the top. Each bet has a `section` field that groups it on the page (Planning, Grooming/refinement/specs, Pointing and sizing, Standup, Code review, Testing and CI hygiene, PR review at scale). Commit and push, GitHub Pages redeploys in about a minute.
