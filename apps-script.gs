// AI-DLC · Bang for the Buck — Apps Script web app
...
function doPost(e) {
  try {
    ...
    const pickNames = (data.picks || []).map(p => {
      ...
      const cLabel = c > 1 ? ` ×${c}` : "";
      return `${p.id}${cLabel} [${p.section}] ${p.name} ($${sub})`;
    }).join(" | ");
...
function doGet() {
  return ContentService.createTextOutput("AI-DLC vote endpoint is live.");
}
